...
const cardDescription = "עסקים שרוצים יותר לקוחות בלי להסתבך עם פרסום";
    ...
    <section id="audience" className="scroll-mt-24 border-y border-white/5 px-4 py-16 sm:px-6 sm:py-20">
      <div className="mx-auto max-w-6xl">
        ...
        <p className="mx-auto mt-3 max-w-xl text-center text-muted">
          אם אתם מזהים את עצמכם כאן — השירות בנוי בדיוק בשבילכם.
        </p>
        <div className="mt-12 grid gap-4 sm:grid-cols-2">
          {audience.map((item) => {
            ...
            <article
              key={item.title}
              className={cn(
                "rounded-2xl border border-white/10 bg-surface/50 p-5 shadow-card transition",
                "hover:border-accent/25 hover:shadow-lg hover:shadow-black/20 sm:p-6",
              )}
            >
              <div className="flex gap-4">
                <span
                  className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-accent/10 text-accent ring-1 ring-accent/20"
                  aria-hidden
                >
...
function DumbbellIcon({ className }: { className?: string }) {
  return (
    <svg className={className} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.75" aria-hidden>
      <path
        d="M6.5 6.5h3v11h-3a2 2 0 0 1-2-2v-7a2 2 0 0 1 2-2Z"
