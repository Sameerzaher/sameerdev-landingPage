...
export function PainSection() {
  return (
    <section id="pain" className="section-y scroll-mt-24">
      <div className="mx-auto max-w-6xl">
        <h2 className="section-heading">אם זה נשמע מוכר — אתה לא לבד</h2>
        <p className="section-sub">שלוש בעיות שחוזרות אצל בעלי עסקים — וכל אחת מהן עולה לך בלקוחות.</p>
        <div className="mt-12 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {cards.map((card) => (
            <article
              key={card.title}
              className="rounded-2xl border border-white/10 bg-surface/60 p-6 shadow-card backdrop-blur-sm transition hover:-translate-y-0.5 hover:border-accent/25 hover:shadow-glow"
            >
